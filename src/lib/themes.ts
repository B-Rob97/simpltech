...
function buildThemeBootstrapScript() {
  ...
  return `(()=>{try{var m={${map}};var k=${JSON.stringify(THEME_STORAGE_KEY)};var guard="simpltech-theme-sync";var r=document.documentElement;var rendered=r.getAttribute("data-theme");var id=null;try{id=localStorage.getItem(k)}catch(e){}if(!id){var match=document.cookie.match(/(?:^|; )simpltech-theme=([^;]*)/);if(match)id=decodeURIComponent(match[1])}if(!m[id])id=${JSON.stringify(DEFAULT_THEME_ID)};var t=m[id];r.setAttribute("data-theme",id);r.setAttribute("data-motion",t[0]);r.setAttribute("data-density",t[1]);r.setAttribute("data-hero",t[2]);r.style.colorScheme=t[3];if(id!==rendered){document.cookie=k+"="+encodeURIComponent(id)+"; path=/; max-age=31536000; SameSite=Lax";var stamped=null;try{stamped=sessionStorage.getItem(guard)}catch(e){}if(stamped!==id){try{sessionStorage.setItem(guard,id)}catch(e){}r.setAttribute("data-theme-reload","1");r.style.visibility="hidden";location.replace(location.href);return}}try{sessionStorage.removeItem(guard)}catch(e){}r.style.visibility=""}catch(e){}})()`;
}
