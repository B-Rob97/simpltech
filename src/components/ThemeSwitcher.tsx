    ...
    <div ref={rootRef} className="relative">
      <div className="flex items-center">
        <button
          ...
          disabled={refreshing}
          onClick={cycleTheme}
          aria-label={`Theme ${theme.label}. Cycle to the next design.`}
          className="inline-flex max-w-[9.5rem] items-center gap-2 rounded-[var(--radius-button)] border-[length:var(--border-width)] border-foreground/15 bg-foreground/5 px-2.5 py-1.5 text-left text-foreground transition-colors hover:border-foreground/30 hover:bg-foreground/10 disabled:opacity-60 sm:max-w-none sm:px-3"
        >
          <span
            aria-hidden
            data-theme={theme.id}
            className="h-2.5 w-2.5 shrink-0 rounded-[2px] bg-[color:var(--accent)] shadow-[0_0_0_1px_color-mix(in_oklab,var(--foreground)_20%,transparent)]"
          />
          <span className="min-w-0">
            <span className="block truncate font-[family-name:var(--font-display)] text-[11px] font-semibold leading-none tracking-wide sm:text-xs">
              {theme.label}
            </span>
            <span className="mt-0.5 hidden text-[10px] leading-none text-foreground/55 sm:block">
              Cycle look
            </span>
          </span>
        </button>
