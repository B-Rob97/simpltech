  ...
  return (
    ...
    >
          ...
          <>
            ...
            <Reveal delay={0.08}>
              <p className="mt-4 max-w-2xl text-base leading-relaxed text-foreground/65 sm:text-lg">
                A sample of client work and internal platforms — from
                storefronts to operations software.
              </p>
            </Reveal>
          </>
        ) : (
      ...
      <div className="relative z-10 mx-auto max-w-[var(--content-max)] px-5 pb-[var(--section-space)] sm:px-8">
        <ul className="night-street-grid">
          {projects.map((project, index) => {
            const content = (
              <>
                <NightSignalCover
                  projectId={project.id}
                  className="night-stack-thumb"
                />
                <div className="night-stack-copy">
                ...
                <Reveal delay={index * 0.05}>
                  {project.href ? (
                    <a
                      ...
                      className="night-street-card group block"
                    >
                      {content}
                    </a>
                  ) : (
                    <div className="night-street-card">{content}</div>
                  )}
                </Reveal>
              </li>
            );
